module.exports = {
    ...
    'rules': {
        ...
        'import/no-webpack-loader-syntax': 'warn', // 使用 webpack loader 语法 import 文件时发出警告
        'react/jsx-uses-react': 'error', // 使用 JSX 语法的文件需要引入 react 模块
        'react/jsx-uses-vars': 'error', // JSX 中不允许使用 var 定义变量
        'react/no-string-refs': 'warn', // 当使用字符串形式引用组件元素时发出警告 (<div ref="div"/>)
        'react/prop-types': 'off', // 不检查 PropTypes
        'react/display-name': 'warn', // 使用 createReactClass 创建组件时没有 displayName 属性将会警告
        'react/no-unescaped-entities': 'off' // JSX 中不检查转义字符串
    }
};
